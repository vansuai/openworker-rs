...
//!   1. picks a free localhost port and starts the Rust `ocw-server` as a managed
...
///   2. The bundled sidecar shipped via Tauri `resources` (production): the
///      `sidecar/` folder lands in Contents/Resources on macOS and in the install dir
///      (next to the app exe) on Windows. Prefers `ocw-server`, falls back to
///      `openworker-server` during the dual-track cutover.
///   3. Dev fallback: the Rust workspace target dir, relative to this crate
///      (`src-tauri` → `crates/target/debug/ocw-server` or `release` depending on profile).
fn server_bin() -> PathBuf {
    ...
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            ...
            let mut bases = vec![dir.join("sidecar")];
            if let Some(contents) = dir.parent() {
                bases.push(contents.join("Resources").join("sidecar"));
            }
            for base in bases {
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            // 1. Start the Rust server sidecar on the chosen port (inherits our env).
