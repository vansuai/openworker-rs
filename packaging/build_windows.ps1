...
Require-Cmd rustc
Require-Cmd npm
$SidecarKindEarly = if ($env:OCW_SIDECAR) { $env:OCW_SIDECAR } else { "rust" }
if ($SidecarKindEarly -eq "python" -and -not (Test-Path $PyInst)) {
    throw "PyInstaller not found at $PyInst (required when OCW_SIDECAR=python). Create the venv and install deps (see header)."
}
...
Write-Host "==> [1/4] Building Rust server (ocw-server.exe)" -ForegroundColor Cyan
$CargoToml = Join-Path $Platform "crates\Cargo.toml"
& cargo build -p ocw-server --manifest-path $CargoToml --release
if ($LASTEXITCODE -ne 0) { throw "Rust build failed (exit $LASTEXITCODE)" }
Write-Host "    -> $(Join-Path $Platform 'crates\target\release\ocw-server.exe')"

$SidecarKind = if ($env:OCW_SIDECAR) { $env:OCW_SIDECAR } else { "rust" }
...
if ($SidecarKind -eq "python") {
    Write-Host "==> [2/4] PyInstaller: bundling openworker-server ($Triple) [OCW_SIDECAR=python]" -ForegroundColor Cyan
    ...
    Write-Host "==> [3/4] staging Python sidecar resources" -ForegroundColor Cyan
    $Src = Join-Path $Here "dist\openworker-server"
    Copy-Item -Recurse -Force $Src\* $Dst
    # Transition alias so Tauri server_bin() (looks for ocw-server.exe) finds it.
    Copy-Item -Force (Join-Path $Dst "openworker-server.exe") (Join-Path $Dst "ocw-server.exe")
} else {
    if (-not (Test-Path $PyInst)) {
        Write-Host "    (PyInstaller not required for default Rust sidecar)" -ForegroundColor DarkGray
    }
    Write-Host "==> [2/4] Skipping PyInstaller (default Rust sidecar)" -ForegroundColor Cyan
    Write-Host "==> [3/4] staging Rust ocw-server.exe into sidecar\" -ForegroundColor Cyan
    $RustExe = Join-Path $Platform "crates\target\release\ocw-server.exe"
    Copy-Item -Force $RustExe (Join-Path $Dst "ocw-server.exe")
}
Write-Host "    -> $Dst"
