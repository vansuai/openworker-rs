...
#
#   1. Build the Rust ocw-server (default sidecar). Optional: OCW_SIDECAR=python
#      stages the PyInstaller openworker-server instead (rollback).
...
#   - For OCW_SIDECAR=python only: a Python venv at .venv (repo root) with this package
#     installed editable, plus the build-only deps:
#       python3 -m venv .venv
...
echo "==> [1/6] Building Rust server (ocw-server)"
cargo build -p ocw-server --manifest-path "$PLATFORM/crates/Cargo.toml" --release
echo "    -> $PLATFORM/crates/target/release/ocw-server"

# Default sidecar is Rust ocw-server. Set OCW_SIDECAR=python to stage the PyInstaller
# openworker-server instead (rollback / dual-track transition).
SIDECAR_KIND="${OCW_SIDECAR:-rust}"
mkdir -p "$GUI/src-tauri/binaries"
rm -rf "$GUI/src-tauri/binaries/sidecar" "$GUI/src-tauri/binaries/openworker-server-$TRIPLE"
mkdir -p "$GUI/src-tauri/binaries/sidecar"

if [ "$SIDECAR_KIND" = "python" ]; then
  echo "==> [2/6] PyInstaller: bundling openworker-server ($TRIPLE) [OCW_SIDECAR=python]"
  "$PLATFORM/.venv/bin/pyinstaller" --noconfirm --clean \
    --distpath "$HERE/dist" --workpath "$HERE/build" "$HERE/openworker-server.spec"

  echo "==> [3/6] staging Python sidecar resources"
  ...
  # Also copy as ocw-server alias so Tauri server_bin() finds it during transition.
  ln -sf openworker-server "$GUI/src-tauri/binaries/sidecar/ocw-server" 2>/dev/null \
    || cp "$GUI/src-tauri/binaries/sidecar/openworker-server" "$GUI/src-tauri/binaries/sidecar/ocw-server"
  SIDECAR_ENTRY="openworker-server"
else
  echo "==> [2/6] Skipping PyInstaller (default Rust sidecar; set OCW_SIDECAR=python to force)"
  echo "==> [3/6] staging Rust ocw-server into sidecar/"
  cp "$PLATFORM/crates/target/release/ocw-server" "$GUI/src-tauri/binaries/sidecar/ocw-server"
  chmod +x "$GUI/src-tauri/binaries/sidecar/ocw-server"
  SIDECAR_ENTRY="ocw-server"
fi
...
if [ -n "${APPLE_SIGNING_IDENTITY:-}" ]; then
  ...
  find "$SIDECAR" -type f ! -name "$SIDECAR_ENTRY" ! -name "ocw-server" \
  ...
  codesign --force --sign "$APPLE_SIGNING_IDENTITY" --timestamp --options runtime \
    --entitlements "$GUI/src-tauri/entitlements.plist" "$SIDECAR/$SIDECAR_ENTRY"
  if [ -f "$SIDECAR/ocw-server" ] && [ "$SIDECAR_ENTRY" != "ocw-server" ]; then
    codesign --force --sign "$APPLE_SIGNING_IDENTITY" --timestamp --options runtime \
      --entitlements "$GUI/src-tauri/entitlements.plist" "$SIDECAR/ocw-server"
  fi
fi
